<template>
	...
	<a-card :bordered="false" class="header-solid h-full" :bodyStyle="{padding: 0,}">
		<template #title>
          ...
          show-search
          :allowClear="true"
					v-model="selectBigVersion"
					style="width:100px"
					@change="bigVersionFilter()"
					>
						<a-select-option v-for="item in bigVersionOption" :key="item" :value="item">{{item}}</a-select-option>
					</a-select>
					<a-select
          show-search
          :allowClear="true"
    ...
    <div>
      <a-popconfirm
        :title="stringSplicing('确认删除该备份版本吗?  ',selectBackup.build)"
        ok-text="Yes"
        cancel-text="No"
        @confirm="deleteBackupVersion()"
      >
        <a-button type="dasked" size="small">删除备份版本</a-button>
      </a-popconfirm>
	  ...
	  <a-button v-show="selectBackupVersionOfflineInstaller!=''" >Copy</a-button>
	</div>
    <p></p>
...
import { myNotification } from '../../mixin/notification'
    ...
    props:{},
      ...
      const sdxbackupService = new SdxBackupService()
        ...
        selectBigVersion:'所有版本',
        selectBackupVersionRemark:'',
        selectBackupVersionOfflineInstaller:'',
        allBackupVersion:[],
        allBackupVersion_copy:[],
        bigVersionOption:['所有版本','关键版本']
      ...
      dataFromBackupInit(){
        this.allBackupVersion = []
          ...
          res.data = res.data.sort(function(a,b){
            ...
            if(this.bigVersionOption.indexOf(element.version)==-1){
							this.bigVersionOption.push(element.version)
            }
					})
					this.selectBackupVersionId = this.allBackupVersion[0].value
          
          this.allBackupVersion_copy = this.allBackupVersion
      ...
      bigVersionFilter(){
        ...
        if(this.selectBigVersion == '所有版本'){
					this.allBackupVersion = this.allBackupVersion_copy
        }
        else if(this.selectBigVersion == '关键版本'){
					this.allBackupVersion_copy.forEach(item=>{
  					if(item.text.indexOf('*')!=-1){
  						_filterVersion.push(item)
            }
          })
          this.allBackupVersion = _filterVersion	
        }
        else{
          this.allBackupVersion_copy.forEach(item=>{
  					if((item.text||'').split('.')[0] == this.selectBigVersion){
  						_filterVersion.push(item)
            }
          })
          this.allBackupVersion = _filterVersion	
