<template>
	...
	<a-card :bordered="false" class="header-solid h-full" :bodyStyle="{paddingTop: 0, paddingBottom: '16px' }">
		<template #title>
			...
			<span style="font-size:17px">Workload & components</span>
			<a-button type="dashed" size="small" shape="round" style="float:right;">数据导出到Excel文件</a-button>
		</template>
		<span style="font-size:17px">Project types:</span><p></p>
		<a-row :gutter="[24, 24]">
			<a-col :span="24" v-for="(item,id) in allWorkloadData" :key=id>
				<a-card :bordered="false" class="card-billing-info">
					<div class="col-info">
						<a-checkbox @change="cb_change(item.title,true)" :checked="judgeIsChecked(item.id,false)"></a-checkbox>	
						<span style="margin-left:7px;" class="span_title"  @click="pluginDetail(item.id,false)">{{item.title}}</span><p></p>
						<!--<a-icon type="more" @click="pluginDetail(item.id,false)"></a-icon>-->
						<p>{{InitWorkload(item.id).description}}</p>
						
						<a-descriptions title="Required (isOptional = false 不显示)" :column="4" v-if="judgeIsOptional(item.includePackages,true).length!=0" style="margin-left:10px;">
						...
						<p></p>
						<a-descriptions title="Optional (isOptional = true 显示但禁用)" :column="2" style="margin-left:10px;" v-if="judgeIsOptional(item.includePackages,flase).length!=0">
		...
		<span style="font-size:17px">Dependencies/features:</span><p></p>
		<a-row :gutter="[24, 24]">
			<a-col :span="24">
				<a-card :bordered="false" class="card-billing-info" style="height:50px;"></a-card>
			</a-col>
			
		</a-row>
		<a-button style="margin-left:0px;" size="small" @click="selectAll(true)">Select All</a-button>
		...
		<p style="margin-left:0px;">* Select All or Unselect All does not change the selection of the "Environment dependency" component (runtime components).
		</p>
		<a-drawer
			placement="right"
			:closable="false"
