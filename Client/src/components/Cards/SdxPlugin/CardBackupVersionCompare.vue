<template>
	...
	<a-card :bordered="false" class="header-solid h-full" :bodyStyle="{padding: 0,}">
        ...
        <a-col :span="12" :md="11">
          ...
          <a-select
            v-model="PrimarySelectAttribute"
  					style="width:150px"
            :maxTagCount="5"
            show-search
            @change="selectAttributeWayChange()"
  					>
						<a-select-option  v-for="item in PrimarySelectAttributeOption" :key="item.value" :value="item.value">{{item.text}}</a-select-option>
					</a-select>
				</a-col>
        <a-col :span="12" :md="1">
  ...
  import {CodeDiff} from 'v-code-diff'
	...
	export default ({
      ...
      const sdxbackupService = new SdxBackupService()
        ...
        //****要进行比较的属性
        selectCompareAttribute:[
          'id','title','description','packageType','releaseNotes','isRequireLicenseAcceptance',
				  'hasLicense','license','licenseUrl','kind','tags','authors','minClientVersion','compatibleVersion',
          'summary','isPrerelease','hasEmbeddedIcon','hasReadme','envDependencies','preDependencies','includePackages'
        ],
        not_easily_change_Attribute:[
          'id','title','description','packageType','releaseNotes','isRequireLicenseAcceptance',
				  'hasLicense','license','licenseUrl','kind','tags','authors','minClientVersion','compatibleVersion',
          'summary','isPrerelease','hasEmbeddedIcon','hasReadme','envDependencies','preDependencies','includePackages'
        ],
        ...
        change_every_time_Attribute:[
					'id','title','version','md5','installBytes','zipBytes','published','downloads'
        ],
        PrimarySelectAttribute:'not_easily_change',
        PrimarySelectAttributeOption:[
					{value:'title_des',text:'标题和描述'},
          {value:'not_easily_change',text:'不轻易改变的属性'},
          {value:'change_every_time',text:'每次均改变的属性'},
        ],
      ...
      selectAttributeWayChange(){
        switch(this.PrimarySelectAttribute){
					case 'title_des':
            this.selectCompareAttribute = this.title_des_Attribute
            break
          case 'not_easily_change':
            this.selectCompareAttribute = this.not_easily_change_Attribute
            break
          case 'change_every_time':
            this.selectCompareAttribute = this.change_every_time_Attribute
            break
          default:
            this.selectCompareAttribute = this.not_easily_change_Attribute
            break
        }
        this.selectAttributeChange()
      },
