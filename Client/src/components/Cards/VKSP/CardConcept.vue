<template>
	...
	<a-card :bordered="false" :bodyStyle="{padding: 0,}">
      ...
      <p></p>
      <div >
        <span style="margin-left:20px;">创建人员：</span>
  			<a-select
          ref="select"
          v-model:value="Filter.user"
          style="width: 180px"
          placeholder="创建人员"
          @change="handleChange_Filter"
        >
          <a-select-option v-for="item in userOptions" :key="item.value" :value="item.value">
            <a-avatar v-if="item.avatar!='' && item.avatar!=null" shape="circle" :src="item.avatar" style="margin-left:5px"/>
  					<a-avatar v-else style="background-color:#1890ff;margin-left:5px">{{item.text.charAt(0).toUpperCase()}}</a-avatar>  
            {{item.text}}
          </a-select-option>
    		</a-select>
        ...
        <a-select
          size="small"
          ref="select"
          v-model:value="Filter.selectYear"
          style="width: 100px"
          placeholder="年"
        ...
        <a-select
          size="small"
          ref="select"
          v-model:value="Filter.selectMonth"
      ...
      <template slot="slotsimpleContent" slot-scope="slotsimpleContent">
        <span class="m-0">{{ slotsimpleContent+'...' }}</span>
			</template>
